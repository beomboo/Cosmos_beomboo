...
| 생년월일시 정보 입력 화면 | `project/lib/features/birth_input/` | 🟢 | 생년월일시·성별·MBTI 입력. 제출 시 `pushReplacementNamed`로 계산 중 화면으로 넘어가 스택에서 제거됨(뒤로가기로 재진입 불가). 날짜/시간 `PastelPillButton`은 값만 담은 자체 라벨 위에 `Semantics(excludeSemantics: true, button: true, onTap: ..., label: '태어난 날짜/시간 ...')`로 필드 맥락을 더해 버튼 역할·탭 액션은 유지한 채 라벨만 교체(단순 중첩 Semantics는 라벨이 override가 아니라 concat돼 값이 중복 재생됨을 실측 확인). 자시(23시~01시) 경계 안내: 계산 로직(`four_pillars.dart`의 `midnight` 관법)은 그대로 두고, `_isJasiRange`(hour==23 또는 0)이면서 시간 모름 체크가 꺼져 있을 때만 시간 pill 아래에 순화된 안내 문구를 노출(리서치: `docs/research/운세/입력_온보딩_설계.md`). 제출 버튼 `Text('사주 보러가기 🔮')`는 이모지가 스크린 리더에 읽히지 않도록 `semanticsLabel: '사주 보러가기'` 지정 |
...
| 공용 위젯 | `project/lib/shared/widgets/` | 🟢 | `PastelPillButton`/`PastelToggleRow`/`PastelCard`/`OffscreenShareCapture`(오프스크린 공유 캡처 래퍼)/`ShareCardScaffold`(공유 카드 9:16 공통 스캐폴드) |
