    ...
    return Scaffold(
      body: SafeArea(
        child: Padding(
          ...
          // 랜드스케이프처럼 세로 폭이 좁은 뷰포트(예: 812x375)에서는 아래 고정
          // Column(Spacer 2개 포함)이 실제로 "RenderFlex overflowed" 예외를 낸다
          // (2026-07-14 발견) — birth_input_screen.dart의 ListView처럼 아예 스크롤
          // 전용으로 바꾸는 대신, 뷰포트가 충분히 크면 지금처럼 세로 중앙 정렬을
          // 유지하고 부족할 때만 스크롤되도록 LayoutBuilder+ConstrainedBox(minHeight)
          // +IntrinsicHeight 조합을 쓴다. Spacer는 Scrollable 안에서 무한 높이 제약을
          // 만나 에러를 내므로 Expanded(SizedBox)로 교체한다.
          child: LayoutBuilder(
